...
// DYSEKT patch: thread_local bridge between VST3 wrapper and DysektProcessor.
// Implemented in PluginProcessor.cpp. Called only within the scope of process().
...
public:
    ...
    Steinberg::int32 PLUGIN_API getBusCount (Vst::MediaType type, Vst::BusDirection dir) override
    {
        ...
        if (type == Vst::kEvent)
        {
           #if JucePlugin_WantsMidiInput
            if (dir == Vst::kInput)
                return 2; // DYSEKT: port 0 = slicer, port 1 = DY-SFP (SF2/SFZ player)
           #endif
                                   ...
                                   Steinberg::int32 index, Vst::BusInfo& info) override
    {
        ...
        if (type == Vst::kEvent)
        {
            ...
            if (dir == Vst::kInput && index == 0)
            {
                ...
                toString128 (info.name, "DYSEKT");
                info.busType = Vst::kMain;
                return kResultTrue;
            }

            // DYSEKT patch: second MIDI input bus for the SF2/SFZ player
    ...
    tresult PLUGIN_API process (Vst::ProcessData& data) override
    {
        ...
        {
            // DYSEKT patch: split events by busIndex using two MidiEventList wrappers.
            // busIndex 0 -> slicer MidiBuffer (midiBuffer)
            // busIndex 1 -> SF2/SFZ player MidiBuffer (via dysektSetSfPlayerMidiPort)
            MidiEventList slicerEvents, sfpEvents;
            ...
            for (Steinberg::int32 i = 0; i < numEvts; ++i)
            {
                ...
                if (data.inputEvents->getEvent (i, e) != Steinberg::kResultOk)
                    continue;
                if (e.busIndex == 1)
                    sfpEvents.addEvent (e);
                else
                    slicerEvents.addEvent (e);
            }
            MidiEventList::toMidiBuffer (midiBuffer, slicerEvents);
            juce::MidiBuffer sfpMidiBuffer;
            MidiEventList::toMidiBuffer (sfpMidiBuffer, sfpEvents);
            // Make DY-SFP buffer accessible to DysektProcessor via thread_local bridge.
            // The pointer is valid for the duration of processBlock only.
            dysektSetSfPlayerMidiPort (sfpMidiBuffer.isEmpty() ? nullptr : &sfpMidiBuffer);
        }
       #endif
       ...
       #if JucePlugin_WantsMidiInput
        // DYSEKT patch: clear the thread_local bridge after processBlock returns.
        dysektSetSfPlayerMidiPort (nullptr);
       #endif
