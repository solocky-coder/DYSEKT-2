 ...
 shortcutsPanel (p)
{
 ...
 addChildComponent (shortcutsPanel);
    ...
    // Ch 1 (Slicer) and Ch 2 (SfPlayer) are hardwired — no mode switch needed.
    // The sequencer still needs to know which live channel to echo, handled by
    // setSelectedSfLiveChannels() which is independent of channelOwnerMap.
    arrangeView.onTrackTypeSelected = [this] (TrackType type, bool hasSelection)
    {
        if (activeSlot != SlotContent::Seq) return;
        // SF-player track selected → live input goes to sfzPlayer (ch 2).
        // Slicer/chromatic track selected → live input goes to slicer (ch 1).
        // Nothing selected → silence live input.
        if (! hasSelection)
...
// MidiRouteMode is replaced by channelOwnerMap[].  Ch 1 = Slicer, Ch 2 = SfPlayer
// are hardwired.  This function is kept as a no-op so existing call sites compile.
void DysektEditor::syncMidiRouteMode()
{
    // Nothing to do — channelOwnerMap in PluginProcessor handles routing.
}
