...
void SliceControlBar::mouseDown (const juce::MouseEvent& e)
{
 ...
 // Ch 1 (Slicer) and Ch 2 (SfPlayer) are hardwired — never available to chromatic.
 // Ch 3-16 owned by Sf2Preset are also unavailable.
 juce::StringArray names; names.add ("Off");
 for (int i = 3; i <= 16; ++i)
 {
     const auto owner = processor.getChannelOwner (i);
     if (owner == DysektProcessor::ChOwner::Sf2Preset)
         continue;  // owned by an SF2 preset — skip
